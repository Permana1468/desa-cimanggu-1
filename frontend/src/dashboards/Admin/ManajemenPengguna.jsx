                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-white/[0.03]">
                            {isLoading ? (
                                <tr><td colSpan="4" className="p-12 text-center text-text-tertiary font-bold uppercase tracking-widest animate-pulse">Memuat Database...</td></tr>
                                ...
                                <tr key={user.id} className="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td className="p-6 text-[13px] text-text-muted font-medium">{user.unit_detail || '-'}</td>
                                    <td className="p-6 pr-8">
                                        <div className="flex justify-center gap-2.5 opacity-40 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => handleResetPassword(user.id, user.username)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-text-muted hover:bg-gold-light hover:text-gold transition-all shadow-lg"><KeyRound size={15} /></button>
                                            <button onClick={() => handleOpenModal('edit', user)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-text-muted hover:bg-white/10 hover:text-text-main transition-all shadow-lg"><Edit size={15} /></button>
                                            <button onClick={() => handleDelete(user.id)} className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-text-muted hover:bg-red-500/20 hover:text-red-500 transition-all shadow-lg" disabled={user.username === 'admin'}><Trash2 size={15} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            >
                <form id="userForm" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        >
                            {roleOptions.map(r => (
                                <option key={r} value={r} className={theme === 'dark' ? 'bg-[#0f172a]' : 'bg-white'}>{r.replace('_', ' ')}</option>
                            ))}
                        </select>
                    </div>
